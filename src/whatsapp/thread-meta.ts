...
/** Parameters shared by the single and bulk lead-update paths. */
...
/**
 * Apply a single lead update within an **already-open** transaction client.
 * Performs: SELECT previous → upsert meta → replace tags → log transitions.
 *
 * Must be called inside a BEGIN…COMMIT block owned by the caller.
 * Exported so `bulk-lead.ts` can reuse the exact same write logic
 * inside a shared transaction (no copy-paste, no drift).
 */
