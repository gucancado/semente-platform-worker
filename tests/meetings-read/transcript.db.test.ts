...
async function seed(ws: string): Promise<number> {
  const { rows } = await pool.query(
    `INSERT INTO episodes (fonte, external_source, external_id, title, occurred_at, duration_seconds, workspace_id, participants, metadata)
     VALUES ('reuniao','vexa','x','R','2026-07-10T12:00:00Z',34,$1,'[{"name":"Gustavo","email":null}]','{}') RETURNING id`, [ws]);
...
test('getMeetingTranscript devolve turns do episódio do workspace certo', async () => {
