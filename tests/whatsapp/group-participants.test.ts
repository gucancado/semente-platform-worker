...
test('parseParticipants normaliza o jid pro mesmo formato de messages.author', () => {
  ...
  assert.deepEqual(out, [
    { phone: '+5531999998888', isAdmin: true, isLid: false },
    { phone: '+5531988887777', isAdmin: false, isLid: false },
    { phone: '+5531977776666', isAdmin: false, isLid: false },
  ]);
});
...
test('parseParticipants resolve LID pelo campo alt (casa com messages.author)', () => {
  ...
  assert.deepEqual(out, [{ phone: '+5531999998888', isAdmin: true, isLid: false }]);
});
...
test('parseParticipants marca isLid quando o LID vem SEM alt (não é telefone)', () => {
  ...
  assert.deepEqual(out, [{ phone: '+12345', isAdmin: false, isLid: true }]);
});
...
test('upsertParticipants faz um upsert por pessoa e devolve a contagem', async () => {
  ...
  const n = await upsertParticipants(pool, 7, [
    { phone: '+5531999998888', pushName: 'Gustavo', isAdmin: true, isLid: false },
    { phone: '+5531988887777', pushName: null, isAdmin: false, isLid: false },
  ]);
  assert.equal(n, 2);
  assert.equal(calls.length, 2);
  assert.match(calls[0].sql, /ON CONFLICT \(group_id, phone\)/);
  assert.match(calls[0].sql, /last_seen_at = NOW\(\)/);
  assert.deepEqual(calls[0].params, [7, '+5531999998888', 'Gustavo', true, false]);
});
...
test('listParticipants mapeia snake→camel e serializa a data', async () => {
  const pool = {
    query: async () => ({ rows: [{
      phone: '+5531999998888', push_name: 'Gustavo', is_admin: true, is_lid: false,
